...
class Solution {
  public:
    std::vector<int> countLessEq(std::vector<int>& a, std::vector<int>& b) {
        std::sort(b.begin(), b.end());
        std::vector<int> result;

        for (int x : a) {
            // upper_bound gives the first index of element > x
            // So, all elements before it are ≤ x
            int count = std::upper_bound(b.begin(), b.end(), x) - b.begin();
